...
void get() {
    ...
    for(int i=0;i<n;i++) {
        string s;
        cin>>s;
        for(int j=0;j<n;j++) a[i*n+j][n*n]^=(s[j]=='w');
    }
    for(int i=0;i<n;i++) {
        for(int j=0;j<n;j++) {
            a[i*n+j][i*n+j]=1;
            if(i-1>=0) a[(i-1)*n+j][i*n+j]=1;
            if(i+1<n) a[(i+1)*n+j][i*n+j]=1;
            if(j-1>=0) a[i*n+j-1][i*n+j]=1;
            if(j+1<n) a[i*n+j+1][i*n+j]=1;
        }
    }
}
